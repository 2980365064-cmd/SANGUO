...
export function CharacterDetail() {
  ...
  useEffect(() => {
    if (!state.characterFocusName) return;
    const focus = gameState.characters.find((character: Character) => character.name === state.characterFocusName) || null;
    if (focus) setSelectedChar(focus);
  }, [state.characterFocusName, gameState.characters]);
  ...
  return (
    <SceneShell scene="character">
      <AppFrame className="info-page info-page-character" title="人物档案" eyebrow="人物关系 · 心迹与反应" back={<ActionSealButton priority="ghost" onClick={() => navigate(state.characterReturnPage)}><ArrowLeft/> 返回前页</ActionSealButton>} actions={<div className="filter-tabs">
