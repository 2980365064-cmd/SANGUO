...
"""PyInstaller spec for 汉祚再兴·刘备传桌面包。

打 onedir（首选）：
    pyinstaller SanguoLiubei.spec

打 onefile（单文件，启动慢）：
    pyinstaller --onefile SanguoLiubei.spec
    （或改下方 EXE/COLLECT 段；spec 默认走 onedir）

产物：dist/SanguoLiubei/
- macOS：.app bundle 由 BUNDLE 段生成（dist/SanguoLiubei.app）。
- Windows/Linux：dist/SanguoLiubei/ 整目录可分发。
"""
...
exe = EXE(
    ...
    strip=False,
    upx=False,
    console=False,  # pywebview 套壳无需终端窗口；debug 需看日志时改 True 或跑 dist/.../SanguoLiubei 二进制
...
if sys.platform == "darwin":
    app = BUNDLE(
        coll,
        name="SanguoLiubei.app",
        icon=None,
        bundle_identifier="com.local.sanguoliubei",
        info_plist={
