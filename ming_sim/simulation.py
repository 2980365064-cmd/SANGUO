...
def build_simulator_payload(
    ...
    army_rows = [
        dict(r) for r in db.conn.execute(
            "SELECT name,station,station_node,theater,commander,controller,troop_type,manpower,"
            "maintenance_per_turn,supply,supply_turns,morale,training,equipment,arrears,mobility,"
            "loyalty,fatigue,discipline,hazard_turns,hazard_combat_multiplier,"
            "supply_combat_multiplier,starvation_turns,status,owner_power "
            "FROM armies WHERE active = 1 ORDER BY id"
        ).fetchall()
    ]
    recent_battles = []
    for battle in db.conn.execute(
        "SELECT id, turn, node_id, result FROM battles WHERE turn>=? ORDER BY id DESC LIMIT 12",
        (max(0, int(state.turn) - 1),),
    ).fetchall():
        try:
            battle_result = json.loads(str(battle["result"] or "{}"))
        except (TypeError, ValueError, json.JSONDecodeError):
            battle_result = {}
        recent_battles.append({
            "id": int(battle["id"]),
            "turn": int(battle["turn"]),
            "node_id": str(battle["node_id"]),
            "winner": battle_result.get("winner"),
            "final_probability": battle_result.get("final_probability"),
            "random_roll": battle_result.get("random_roll"),
            "commander_fates": battle_result.get("commander_fates") or [],
        })
    external_power_actions = []
    for row in db.conn.execute(
        """
        SELECT turn, power_id, action_type, action_json, score, status, result
        FROM power_ai_actions WHERE turn>=? ORDER BY turn DESC, id
        """,
        (max(0, int(state.turn) - 1),),
    ).fetchall():
        try:
            ...
            action, result = {}, {}
        external_power_actions.append({
            "turn": int(row["turn"]),
            "power_id": str(row["power_id"]),
    ...
    return {
        ...
        "army_orders": db.list_army_orders(int(state.turn)) if hasattr(db, "list_army_orders") else [],
        "active_sieges": db.list_sieges("active") if hasattr(db, "list_sieges") else [],
        "recent_battles": recent_battles,
        "external_power_actions": external_power_actions,
        ...
        " army_orders 与 external_power_actions 均由硬规则结算；不得重复输出战斗伤亡、改换领土或无视补给。",
    }
