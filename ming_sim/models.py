...
class Character:
    ...
    intrigue: int = 50  # 谋略
    learning: int = 50  # 学识
    leadership: int = 50  # 统率（三国版）
    intelligence: int = 50  # 智略（三国版）
    politics: int = 50  # 政治（三国版）
    charisma: int = 50  # 魅力（三国版）
    ambition: int = 50  # 野心（动态人格）
    closeness_to_liu_bei: int = 0  # 对刘备亲密度
    core_tier: str = "3"  # S | 1 | 2 | 3
    location: str = ""
...
class Event:
    ...
    # 三国历史事件卡：角色候补、硬前提与可改写结果均是数据，不交由 LLM 凭空补齐。
    roles: Dict[str, object] = field(default_factory=dict)
    hard_conditions: Dict[str, object] = field(default_factory=dict)
...
class Army:
    ...
    discipline: int = 50
    hazard_turns: int = 0
    hazard_combat_multiplier: float = 1.0
    hazard_mobility_multiplier: float = 1.0
    starvation_turns: int = 0
    supply_combat_multiplier: float = 1.0
    supply_last_settled_turn: int = 0
...
class GameState:
    ...
    stage: str = "流亡军"
    collapse_turns: int = 0
    chengdu_crisis_turns: int = 0
