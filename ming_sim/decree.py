...
from ming_sim.context import ENDING_LABELS, ENDING_ONGOING, victory_status
...
def _settle_after_narrative(
    ...
    # 8) 结局判定只认规则层：刘备死亡、益州核心、政权瓦解、一统或 223.4 收束。
...
def _generate_ending_summary(
    ...
    rule_timeline = list(outcome.get("timeline") or [])
    if rule_timeline:
        timeline = sorted(
            [*timeline, *rule_timeline],
            key=lambda item: (int(item.get("turn") or 0), str(item.get("kind") or "")),
        )
    summary_text = ""
    try:
    ...
    if not summary_text:
        bits = [str(outcome.get("review") or outcome.get("summary") or "")]
        for c in chapters[-6:]:
