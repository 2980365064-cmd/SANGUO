...
class _StateMixin:
    ...
    def save_state(self, state: GameState) -> None:
        self.conn.execute(
            ...
            (id, year, period, turn, turn_phase, ended, ending_status, stage, collapse_turns, chengdu_crisis_turns)
            VALUES (1, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ON CONFLICT(id) DO UPDATE SET year = excluded.year, period = excluded.period,
                ...
                stage = excluded.stage, collapse_turns = excluded.collapse_turns,
                chengdu_crisis_turns = excluded.chengdu_crisis_turns
            """,
            (
                state.year, state.period, state.turn, state.turn_phase,
                1 if state.ended else 0, state.ending_status, state.stage,
                state.collapse_turns, state.chengdu_crisis_turns,
            ),
        )
    ...
    def load_state(self, start_ym: str = "") -> GameState:
        row = self.conn.execute(
            """
            SELECT year, period, turn, turn_phase, ended, ending_status,
                   stage, collapse_turns, chengdu_crisis_turns
        ...
        state = GameState(
            ...
            stage=str(row["stage"] or "流亡军"),
            collapse_turns=int(row["collapse_turns"] or 0),
            chengdu_crisis_turns=int(row["chengdu_crisis_turns"] or 0),
        )
        if metrics:
        ...
        for account in account_rows:
            ...
            if account_name in state.metrics:
