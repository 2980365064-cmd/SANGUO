...
class _SeedMixin:
    def seed_static_data(self) -> None:
        ...
        for character in self.content.characters.values():
            ...
            self.conn.execute(
                ...
                (name, office, office_type, faction, aliases, personal_skills, loyalty, ability, integrity, courage, style,
                 ...
                 leadership, intelligence, politics, charisma, ambition, closeness_to_liu_bei, core_tier,
                ...
                (
                    ...
                    character.charisma,
                    character.ambition,
                    character.closeness_to_liu_bei,
                    character.core_tier,
        ...
        for relation in self.content.opening_relations.get("relations", []):
            power_a, power_b = sorted((str(relation["power_a"]), str(relation["power_b"])))
            self.conn.execute(
                """
                ...
                (
                    power_a,
                    power_b,
                    int(relation.get("public_relation") or 0),
                    int(relation.get("trust") or 0),
                    json.dumps(relation.get("obligations") or [], ensure_ascii=False),
                    json.dumps(relation.get("territorial_claims") or {}, ensure_ascii=False),
                    json.dumps(relation.get("marriage_hostages") or {}, ensure_ascii=False),
                    int(relation.get("military_coordination") or 0),
                    str(relation.get("status") or "neutral"),
                ),
            )
        for treaty in self.content.opening_relations.get("treaties", []):
            self.conn.execute(
                """
                INSERT OR IGNORE INTO diplomacy_treaties
                (treaty_key, proposer, target, treaty_type, terms, start_turn, end_turn, status)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    str(treaty.get("treaty_key") or ""),
                    str(treaty["proposer"]),
                    str(treaty["target"]),
                    str(treaty["treaty_type"]),
                    json.dumps(treaty.get("terms") or {}, ensure_ascii=False),
                    int(treaty.get("start_turn") or 1),
                    treaty.get("end_turn"),
                    str(treaty.get("status") or "active"),
                ),
            )
        for relation in self.content.families:
            self.conn.execute(
                """
                INSERT OR IGNORE INTO family_relations
                (person_a, person_b, relation_type, status, start_year, end_year,
                 political_effect, succession_risk, source)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    str(relation["person_a"]),
                    str(relation["person_b"]),
                    str(relation["relation_type"]),
                    str(relation["status"]),
                    int(relation.get("start_year") or 0),
                    int(relation.get("end_year") or 0),
                    str(relation.get("political_effect") or ""),
                    str(relation.get("succession_risk") or ""),
        ...
        if is_fresh_armies_seed:
            for army in self.content.armies.values():
                self.conn.execute(
                    """
                    INSERT INTO armies
                    (id, name, station, station_node, theater, commander, controller, troop_type, troop_composition, manpower,
                     maintenance_per_turn, supply, supply_turns, morale, training, equipment, arrears,
                     mobility, loyalty, fatigue, experience, discipline, hazard_turns, specialties, status, owner_power)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    (
                        army.id,
                        ...
                        army.discipline,
                        army.hazard_turns,
                        json.dumps(army.specialties, ensure_ascii=False),
                        army.status,
                        army.owner_power,
                    ),
                )
            # 开局持械（army_arms）延后到 init_weapons 之后 seed，见 _seed_opening_army_arms。
        has_xiakou_right = self.conn.execute(
            "SELECT 1 FROM diplomacy_treaties WHERE treaty_type='驻军权' AND proposer='liu_qi' AND target='liu_bei'"
        ).fetchone()
        if has_xiakou_right is None:
            self.conn.execute(
                """
                INSERT INTO diplomacy_treaties
                (proposer, target, treaty_type, terms, start_turn, end_turn, status)
                VALUES (?, ?, ?, ?, ?, NULL, 'active')
                """,
                (
                    "liu_qi", "liu_bei", "驻军权",
    ...
    def sync_economy_accounts(self, state: GameState) -> None:
        if not all(account in state.metrics for account in ECONOMY_ACCOUNTS):
            return
        notes = {
    ...
    def ensure_opening_ledger(self, state: GameState) -> None:
        for account in ECONOMY_ACCOUNTS:
            if account not in state.metrics:
                continue
            exists = self.conn.execute(
