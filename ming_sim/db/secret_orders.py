...
def _compact_secret_lines_for_pack(text: object, max_chars: int = 80) -> List[Dict[str, object]]:
    return _SecretOrdersMixin._compact_secret_order_lines(text, max_chars=max_chars)


def build_secret_order_adjudication_pack(
    db,
    state: object,
    order_id: int,
    *,
    ...
    if not order:
        raise ValueError(f"密令不存在：{order_id}")
    minister = str(order.get("minister_name") or "")
    detailed = not viewer or viewer == minister or viewer in {"刘备", "主公"}
    public_summary = {
        "id": int(order["id"]),
        "minister_name": minister,
        "title": str(order.get("title") or ""),
        "status": str(order.get("status") or ""),
        "turn_issued": int(order.get("turn_issued") or 0),
        "due_turn": int(order.get("due_turn") or 0),
        "tags": order.get("tags") or [],
    }
    facts = {
        "scope": "own" if detailed else "public",
        "order": public_summary,
        "progress": _compact_secret_lines_for_pack(order.get("result") or ""),
        "sim_note": _compact_secret_lines_for_pack(order.get("sim_note") or ""),
    }
    if detailed:
        facts["order_detail"] = {
            "content": str(order.get("content") or ""),
            "result": str(order.get("result") or ""),
            "sim_note": str(order.get("sim_note") or ""),
        }
    allowed = ["continue_secret_order", "add_progress_note"]
    status = str(order.get("status") or "")
    due_turn = int(order.get("due_turn") or 0)
    turn = int(getattr(state, "turn", 0))
    if status in {"active", "pending_review"} and due_turn and due_turn <= turn:
        allowed.extend(["submit_for_review", "close_done", "close_failed"])
    return build_adjudication_pack(
        ...
        facts=facts,
        rules={
            "visibility_rule": "承办人/刘备可见详情；其他人物只可见公开暗流摘要。",
            "effect_rule": "密令进展只能写入 result/sim_note；成功、失败或结案必须调用密令结构化流程。",
            "fate_rule": "密令不得直接写人物死亡、被俘、易主或策反成功，除非另有专门规则门槛。",
        },
        allowed_outcomes=allowed,
        forbidden_outcomes=["unlisted_death", "free_defection", "unvalidated_territory_change", "free_secret_success"],
        ai_options=[{"outcome": item, "label": item} for item in allowed],
        audit={"detailed": detailed, "due": bool(due_turn and due_turn <= turn)},
        source_tables=["secret_orders", "characters"],
    )
...
class _SecretOrdersMixin:
    ...
    def _validate_secret_order_assignee(self, minister_name: str) -> None:
        ...
        if row is None:
            raise ValueError(f"密令承办人「{name}」未入军府人物名册，请先召见或登记此人。")
        if (row["power_id"] or "liu_bei") != "liu_bei":
            raise ValueError(f"密令承办人「{name}」不属刘备军府，不能承办密令。")
        ...
        if office_type == "后宫":
            raise ValueError(f"密令承办人「{name}」不属军府任事人物，不能承办密令。")
        if office_type == "candidate" or not office:
            raise ValueError(f"密令承办人「{name}」尚无可任事职分，不能承办密令。")
    ...
    def _append_secret_order_line(
        ...
        if column == "sim_note":
            row = self.conn.execute(
                f"SELECT {column} AS v FROM secret_orders WHERE id = ? AND status IN ('active', 'pending_review')",
                (int(order_id),),
            ).fetchone()
        else:
            row = self.conn.execute(
    ...
    def rush_secret_order(
        ...
        why = (reason or "").strip()[:120] or "奉主公军府方略加急"
        prev = row["result"] or ""
        lines = [ln for ln in prev.split("\n") if ln.strip()]
        if months <= 0:
            lines.append(f"{stamp}[军府即核] {why}；本月即移交密令核议。")
            self.conn.execute(
                """
                UPDATE secret_orders
            ...
            lines.append(f"{stamp}[军府加急] {why}；期限改为 {months} 个月内核议。")
            self.conn.execute(
                """
                UPDATE secret_orders
    ...
    def auto_submit_due_secret_orders(self, state: GameState) -> List[Dict[str, object]]:
        ...
        for row in rows:
            ...
            note = "军府期限已至，移交月末密令核议；据既有查办、风声与盘面定成败。"
            prev = row["result"] or ""
            lines = [ln for ln in prev.split("\n") if ln.strip()]
    ...
    @staticmethod
    def _secret_order_ai_note(result: Dict[str, object]) -> str:
        if result.get("status") == "pending_review":
            pending = dict(result.get("pending_adjudication") or {})
            return f"AI裁判暂停待核议#{int(pending.get('id') or 0)}：{pending.get('reason', '')}"
        proposal = dict(result.get("proposal") or {})
        changes = proposal.get("changes") if isinstance(proposal.get("changes"), list) else []
        note = ""
        for change in changes:
            if isinstance(change, dict) and str(change.get("kind") or "") == "secret_sim_note":
                note = str(change.get("note") or change.get("summary") or "")
                break
        if not note:
            note = "；".join(
                str(proposal.get(key) or "").strip()
                for key in ("reason", "risk_note", "recommended_followup")
                if str(proposal.get(key) or "").strip()
            )
        outcome = str(proposal.get("outcome") or "")
        return f"AI裁判依据：{outcome}。{note}".strip()

    def adjudicate_pending_secret_orders(
        self,
        state: GameState,
        llm_config: object | None = None,
        agno_db: object | None = None,
    ) -> List[Dict[str, object]]:
        """用统一裁决器为待核议密令写推演侧线索；不直接结案或制造高危世界变更。"""
        ...
        rows = self.conn.execute(
            """
            SELECT id, minister_name, title FROM secret_orders
        ...
        for row in rows:
            result = run_adjudication(
                ...
                agno_db=agno_db,
                viewer=str(row["minister_name"] or ""),
            )
            item = {
                "order_id": int(row["id"]),
            ...
            if result.get("status") in {"validated", "pending_review"}:
                note = self._secret_order_ai_note(result)
                if note:
                    self.update_secret_order_sim_note(
                        int(row["id"]),
                        note,
