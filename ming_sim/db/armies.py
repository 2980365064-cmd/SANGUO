...
class _ArmiesMixin:
            ...
            payload.append(
                {
                    ...
                    "supply_turns": int(row["supply_turns"]),
                    "fatigue": int(row["fatigue"]),
                    "starvation_turns": int(row["starvation_turns"]),
                    "supply_combat_multiplier": float(row["supply_combat_multiplier"]),
    ...
    def army_detail(self, raw_name: str) -> str:
        ...
        if maint > 0 and arr > 0:
            ...
            arr_text = f"欠饷{arr}万两"
        composition = self._army_troop_composition(row)
        composition_text = "、".join(f"{k}{v}人" for k, v in composition.items()) or str(row["troop_type"])
        try:
            specialties = json.loads(row["specialties"] or "[]")
        except (TypeError, ValueError):
            specialties = []
        specialty_text = "、".join(str(item) for item in specialties) or "无"
        arms_rows = []
        if hasattr(self, "army_arms_payload"):
            arms_rows = self.army_arms_payload(str(row["id"]))  # type: ignore[attr-defined]
        arms_text = "、".join(
            f"{item['troop_type'] or '未分兵种'}配{item['name']}{item['qty']}"
            for item in arms_rows
        ) or "未见专项军械拨发"
        return (
            f"{row['name']}：驻扎地{row['station']}，统帅{row['commander']}，"
            f"兵种{row['troop_type']}，编制{composition_text}，人数{row['manpower']}人，"
            f"维护费{format_money(monthly_amount(maint))} /{TURN_UNIT}，补给{row['supply']}（约{row['supply_turns']}月），"
            f"士气{row['morale']}，训练{row['training']}，装备{row['equipment']}，"
            f"疲劳{row['fatigue']}，断粮{row['starvation_turns']}月，战力补给倍率{row['supply_combat_multiplier']}，"
            f"{arr_text}，机动{row['mobility']}，忠诚{row['loyalty']}，特性{specialty_text}。"
            f"军械：{arms_text}。"
            f"状态：{row['status']}"
        )
    ...
    def army_roster(self, filter_names: Optional[List[str]] = None, index_only: bool = False) -> str:
        ...
        if index_only:
            ...
            other: List[str] = []
            for row in rows:
                if self._is_player_army_power(row["owner_power"]):
            ...
            if own:
                out.append("我军各部（列序＝id|军名）：")
                out.extend(own)
            if other:
                out.append("敌对/外部军（可见情报，列序＝id|军名）：")
                out.extend(other)
            return "\n".join(out)
        if not rows:
            return ""
        ...
        out = [
            ...
            "我军各部（| 分隔，列序＝军名|驻地|统帅|兵种|兵力|月饷万两|补给|士气|训练|装备|机动|忠诚|欠饷万两|欠饷月数|状态；补给…忠诚为0-100）：",
            *own,
        ]
        if other:
            out.append("敌对/外部军（可见情报，列序＝军名|势力|驻地|统帅|兵种|兵力|状态）：")
            out.extend(other)
        return "\n".join(out)

    def military_intel_brief(self, turn: int, limit: int = 8) -> str:
        """给人物召见注入的军情事实块：索引足够短，细项由 query_army_roster 继续查。"""
        rows = self.conn.execute(
            """
            SELECT id,name,station,commander,troop_type,troop_composition,manpower,
                   supply,supply_turns,morale,training,equipment,fatigue,starvation_turns,
        ...
        lines = ["【军情事实块：人物议军必须以此与 query_army_roster 为准，不得凭空编造兵力、补给、俘虏】"]
        if rows:
            lines.append("重点军队（军名|归属|驻地|统帅|兵种/编制|兵力|补给/月|士气|训练|装备|疲劳|断粮|战力补给倍率|状态）：")
            for row in rows:
                composition = self._army_troop_composition(row)
                comp = "、".join(f"{k}{v}" for k, v in composition.items()) or str(row["troop_type"] or "")
                owner = "我军" if self._is_player_army_power(row["owner_power"]) else str(row["owner_power"] or "外部")
                lines.append("|".join(str(x) for x in (
                    row["name"], owner, row["station"], row["commander"], f"{row['troop_type']}({comp})",
                    row["manpower"], f"{row['supply']}/{row['supply_turns']}", row["morale"], row["training"],
                    row["equipment"], row["fatigue"], row["starvation_turns"], row["supply_combat_multiplier"], row["status"],
                )))
        arms = self.army_held_arms_all() if hasattr(self, "army_held_arms_all") else {}
        if arms:
            lines.append("军械实物（军→兵种→型号数量）：")
            for army_name, troop_map in list(arms.items())[:limit]:
                troop_bits = []
                for troop, weapon_map in troop_map.items():
                    troop_bits.append(f"{troop}:" + "、".join(f"{name}{qty}" for name, qty in weapon_map.items()))
                lines.append(f"- {army_name}：" + "；".join(troop_bits))
        battle_rows = self.conn.execute(
            "SELECT turn,node_id,result FROM battles WHERE turn>=? ORDER BY id DESC LIMIT 5",
            (max(0, int(turn) - 3),),
        ).fetchall()
        if battle_rows:
            lines.append("近月战役与将领处置：")
            for battle in battle_rows:
                try:
                    result = json.loads(battle["result"] or "{}")
                except (TypeError, ValueError):
                    result = {}
                fates = result.get("commander_fates") or []
                fate_text = "；".join(f"{item.get('name')}:{item.get('outcome')}" for item in fates) or "无将领特别处置"
                lines.append(
                    f"- 第{battle['turn']}回合 {battle['node_id']}：胜方{result.get('winner', '未明')}，{fate_text}"
                )
        captives = self.conn.execute(
            """
            SELECT name, location, status_reason
            FROM characters
            WHERE status='imprisoned' AND (status_reason LIKE '%战役%' OR status_reason LIKE '%俘%')
            ORDER BY status_changed_turn DESC, name
            LIMIT 8
            """
        ).fetchall()
        if captives:
            lines.append("战俘/系狱将领：")
            lines.extend(f"- {row['name']}：{row['status_reason'] or '下狱'}，所在{row['location'] or '未明'}" for row in captives)
        lines.append("议军规则：若玩家问某军细项，先调 query_army_roster(names=[军名])；若问军械库存/拨发，须有授权并走 dispatch_arms。")
        return "\n".join(lines)
                    ...
                    if field == "troop_composition":
                        ...
                        if str(row["owner_power"]) == "liu_bei":
                            ...
                            next_comp = {
                                ...
                                if str(key).strip() and int(amount) > 0
                            }
                        else:
                            next_comp = {}
    ...
    def create_armies_from_extraction(
        ...
        for raw in new_armies:
            ...
            elif isinstance(item.get("troop_composition"), dict):
                troop_composition = {
                    str(key).strip(): int(amount)
                    for key, amount in item["troop_composition"].items()
                    if str(key).strip() and int(amount) > 0
                }
            else:
                troop_name = str(item.get("troop_type") or "步卒").strip() or "步卒"
