...
from ming_sim.historical_events import evaluate_historical_event
...
def gather_candidate_events(state: GameState, db: GameDB) -> List[Event]:
    ...
    # 三国历史卡：候选人、时间窗口与失效均由规则层结算；
    # 只有 eligible/adapted 可交给推演层，已死人物不会被 LLM 复活执行原剧情。
    for ev in c.events:
        if ev.id in spawned or ev.trigger_year <= 0:
            continue
        if ev.is_historical and ev.roles:
            lifecycle = evaluate_historical_event(db, state, ev.id)
            if lifecycle["status"] in {"eligible", "adapted"}:
                candidates.append(ev)
            continue
        # 旧自定义剧本仍走 require/trigger_gate 兼容路径。
...
def _guard_structured_battle_deltas(
    db: GameDB,
    state: object,
    army_deltas: object,
) -> Dict[str, object]:
    """结构化战役已经落过伤亡时，拒绝 LLM 在同月对参战军重复改数。"""
    if not isinstance(army_deltas, dict):
        return {}
    protected: set[str] = set()
    rows = db.conn.execute(
        "SELECT attacker_ids, defender_ids FROM battles WHERE turn=? AND status='resolved'",
        (int(getattr(state, "turn", 0)),),
    ).fetchall()
    for row in rows:
        for field in ("attacker_ids", "defender_ids"):
            try:
                values = json.loads(str(row[field] or "[]"))
            except (TypeError, ValueError, json.JSONDecodeError):
                values = []
            if isinstance(values, list):
                protected.update(str(item) for item in values)
    pending_orders = db.conn.execute(
        """
        SELECT army_id, payload FROM army_orders
        WHERE turn=? AND order_type='突袭' AND status IN ('issued', 'resolved')
        """,
        (int(getattr(state, "turn", 0)),),
    ).fetchall()
    for row in pending_orders:
        protected.add(str(row["army_id"]))
        try:
            payload = json.loads(str(row["payload"] or "{}"))
        except (TypeError, ValueError, json.JSONDecodeError):
            payload = {}
        if isinstance(payload, dict):
            protected.update(str(item) for item in (payload.get("defender_ids") or []))
    return {
        str(army_id): changes
        for army_id, changes in army_deltas.items()
        if str(army_id) not in protected
    }
...
def apply_score_extraction(
    ...
    army_deltas_raw = _guard_structured_battle_deltas(
        db, state, extracted.get("army_delta") or {}
    )
