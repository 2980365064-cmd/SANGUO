...
from ming_sim.adjudication import attach_adjudication_runtime, run_monthly_adjudication_batch
...
from ming_sim.paths import SANGUO_SCENARIO_ID, user_data_path
from ming_sim.registry import MinisterRegistry, bind_content as _bind_registry
from ming_sim.sanguo_rules import resolve_army_orders_for_turn
from ming_sim.supply import resolve_supply_orders_for_turn, settle_all_army_supply
from ming_sim.siege import resolve_siege_turn
from ming_sim.battle import resolve_battle_orders_for_turn
from ming_sim.government import government_stage
from ming_sim.historical_events import resolve_world_events_for_turn
from ming_sim.national_focus import advance_all_focuses, advance_all_region_investments
from ming_sim.power_ai import resolve_power_ai_turn
from ming_sim.skills import bind_content as _bind_skills
from ming_sim.tools import build_evidence_gate_context
...
def _find_existing_minister(content: GameContent, name: str) -> Optional[str]:
    ...
    if name in content.characters:
        c = content.characters[name]
        if c.status != "candidate" and c.power_id == "liu_bei":
            return name
    for key, c in content.characters.items():
        if c.status == "candidate" or c.power_id != "liu_bei":
            continue
        if name in (c.aliases or []):
            return key
    return None
...
class GameSession:
    ...
    def refresh_registry(self) -> None:
        """仅按当前盘面与 LLM 配置重建人物 Agent，不触发任何回合生命周期动作。"""
    ...
    def _sync_government_stage(self) -> str:
        controlled_nodes = [
            str(row["id"])
        ...
        stage = government_stage(
            ...
            int(self.state.period),
            {
                ...
                "titles": titles,
                "proclaimed_emperor": self.db.kv_get("proclaimed_emperor") == "1",
            },
        )
        changed = str(getattr(self.state, "stage", "")) != stage
        self.state.stage = stage
        if changed and hasattr(self.state, "metrics"):
            self.db.save_state(self.state)
        return stage
    ...
    def issue_army_order(
        ...
        """提交本月结构化主军令；同军同月唯一性及路线合法性由数据库硬校验。"""
        return self.db.issue_army_order(self.state, army_id, order_type, payload)

    def _resolve_strategic_turn(self, strategic_state: object) -> Optional[Dict[str, object]]:
        """月末结构化战略结算：先行军，再执行补给令，最后结算全军口粮。"""
        if hasattr(self, "state") and hasattr(strategic_state, "metrics"):
            strategic_state.metrics = self.state.metrics
        ...
        if hasattr(self.db, "adjudicate_pending_secret_orders"):
            self.db.adjudicate_pending_secret_orders(strategic_state)
        resolve_world_events_for_turn(self.db, strategic_state)
        setattr(strategic_state, "_monthly_adjudication_batch", run_monthly_adjudication_batch(self.db, strategic_state))
        # 战役/围城可在本月改变刘备生死、郡县归属与军队存续，因此必须在硬规则结算后再补一次终局核定。
        if not hasattr(self, "state"):
            return None
        if bool(getattr(self.state, "ended", False)):
            return None
        setattr(strategic_state, "collapse_turns", int(getattr(self.state, "collapse_turns", 0)))
        setattr(strategic_state, "chengdu_crisis_turns", int(getattr(self.state, "chengdu_crisis_turns", 0)))
        outcome = victory_status(self.db, strategic_state)
        self.state.collapse_turns = int(getattr(strategic_state, "collapse_turns", 0))
        self.state.chengdu_crisis_turns = int(getattr(strategic_state, "chengdu_crisis_turns", 0))
        if outcome.get("status") == "ongoing":
            return None
        self.state.ended = True
        self.state.ending_status = str(outcome["status"])
        self.db.save_ending_summary(
            self.state,
            str(outcome["status"]),
    ...
    def chat(
        ...
        augmented = message
        evidence_context = build_evidence_gate_context(character, self.registry.context, message)
        if evidence_context:
            augmented = (
                f"{evidence_context}\n\n"
    ...
    def resolve_turn(self, decree: str = "", on_event=None, cheat_directive: str = "") -> ResolveResult:
        ...
        """
        if self.db.kv_get("scenario_id") == SANGUO_SCENARIO_ID:
            return self._resolve_sanguo_month()
        ...
        strategic_outcome = self._resolve_strategic_turn(strategic_state)
        if strategic_outcome:
            label = ENDING_LABELS.get(str(strategic_outcome["status"]), "结局")
            ending = (
                f"\n\n【结局·{label}】{strategic_outcome.get('summary', '')}"
                f"\n\n{strategic_outcome.get('review', '')}"
            )
            result.report += ending
            self.last_report = result.report
    ...
    def _resolve_sanguo_month(self) -> ResolveResult:
        """三国月末硬规则结算；不再进入旧双库财政与旧抽取器链。"""
        if self.pending_count() > 0:
            raise ValueError(f"尚有 {self.pending_count()} 道拟令待刘备核定，不能月末结算。")
        if self.state.ended:
        ...
        if reputation["recent"]:
            report_lines.append("仁义口碑：" + "；".join(str(item["summary"]) for item in reputation["recent"]))
        if outcome:
            report_lines.append(str(outcome.get("summary") or outcome.get("review") or "本局至此收束。"))
        hard_report = "\n".join(report_lines)
        report = self._narrate_sanguo_month(hard_report)
        self.db.save_turn_report(self.state, report)
    ...
    def _narrate_sanguo_month(self, hard_report: str) -> str:
        """把已经裁定的规则结果改写成月报；模型不可增删或反写任何盘面事实。"""
        if not self.llm_config.api_key or self.llm_config.model == "test":
            return hard_report
        narrator = Agent(
            name="三国月末讲官",
            id="sanguo-month-narrator",
            session_id=f"sanguo-month-{self.state.turn}",
            model=create_chat_model(
                self.llm_config,
                temperature=0.45,
                max_tokens=min(self.llm_config.max_tokens, 2400),
            ),
            instructions=[
                self.content.season_simulator_prompt,
                "你收到的输入已经由硬规则层最终裁定。只可润色和组织叙事；"
                "不得新增人物伤亡、领土变化、兵力、数值、行动或结果，也不得遗漏原文事实。",
            ],
            markdown=False,
        )
        try:
            narrated = extract_agent_text(narrator.run(hard_report)).strip()
            return narrated or hard_report
        except Exception as error:
            print(f"[WARN] 三国月末叙事生成失败，已退回硬规则报告：{error}")
            return hard_report
    ...
    def advance_without_decree(self) -> None:
        ...
        self._resolve_strategic_turn(self.turn_snapshot())
