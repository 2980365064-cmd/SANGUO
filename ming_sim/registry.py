...
# 所有臣将共有的 agno skill：记忆检索、军府方略入档、密令、召见传人。
# office_default_skills[office].agno_skills 可为后续三国专属 skill 扩展使用。
...
def build_intel_tool_index() -> str:
    return (
        "【可查案卷目录（具体事实先查工具，不得凭空报数）】\n"
        "- 军事：query_military_intel(names=[]) 查军队编制、兵力、补给、疲劳、军械、战役、战俘。\n"
        "- 内政：query_internal_intel(regions=[]) 查郡县粮仓、城防、民心、动乱、商税、道路、投资。\n"
        "- 外交：query_diplomacy_intel(targets=[]) 查势力关系、条约、使臣进展、违约风险。\n"
        "- 人事：query_personnel_intel(names=[]) 查人物状态、官职、任事效果、职位空缺。\n"
        "- 密令：query_secret_intel(scope='own|public') 查自己承办密令或公开暗流摘要。\n"
        "- 月报：query_monthly_intel(section='') 查每月总计章节。\n"
        "- 旧事：search_memories(keywords='', year=0, period=0) 查邸报、起居注、旧事来龙去脉。\n"
        "回答若涉及具体数值、条款、人物状态、战役结果，先查对应案卷；未查到就说未见实据。"
    )


def build_character_cognitive_strategy(character: Character) -> str:
    name = character.name
    office = character.office + character.office_type
    traits = "、".join(character.personal_skills)
    base = {
        "knowledge_scope": "基础案卷全可查；正式操作仍受官职授权。",
        "evidence_rule": "具体事实必须来自工具结果或已知上下文；性格只能影响判断倾向，不能覆盖事实。",
    }
    if "诸葛亮" in name:
        mode = "先查军事、外交、内政、人事，必要时再查旧事；多源交叉后给长期方略。"
        bias = "谨慎周密，重全局、粮道、盟约、人才与长期代价。"
    elif "张飞" in name:
        mode = "优先查军事、敌情、士气、补给；除非玩家追问，不主动深挖外交细账。"
        bias = "直断果敢，偏进攻和士气，但不得隐瞒补给不足、疲劳或战败风险。"
    elif "关羽" in name:
        mode = "优先查军事、名分相关月报、荆州/要冲、士气军纪；外交只查与名义和盟约有关部分。"
        bias = "重忠义、军纪、名分和战略要地，判断不可把未签盟约说成事实。"
    elif "赵云" in name:
        mode = "优先查军事安全、疲劳、撤退路、民心与局部风险；高风险行动补查内政和月报。"
        bias = "稳健护主护民，重保全、撤退条件和不扰百姓。"
    elif "糜竺" in name or "糜芳" in name:
        mode = "优先查内政、粮仓、商税、军资、补给；出兵议题必须核后勤承受力。"
        bias = "重钱粮、商路、补给和家底，不以勇气替代账目。"
    elif any(word in office + traits for word in ("外交", "使", "礼", "盟", "辩")):
        mode = "优先查外交、月报、人事和口碑；军事只查与谈判筹码相关部分。"
        bias = "重条款、信义、违约风险与对方反应。"
    elif int(getattr(character, "intelligence", 50) or 50) >= 80 or int(getattr(character, "politics", 50) or 50) >= 80:
        mode = "先查玩家问题直指分类，再补查一个相关分类交叉验证。"
        bias = "较重全局与风险，回答要区分事实、推断和建议。"
    elif int(getattr(character, "martial", 50) or 50) >= 80:
        mode = "优先查军事，必要时补查补给、郡县或月报。"
        bias = "重战机、士气和统兵执行，但不能越过规则制造胜负。"
    else:
        mode = "先查玩家问题直指分类；不熟领域少下断语，可建议传召更合适人物。"
        bias = "以本职和性格立论，未知处不装作知道。"
    return (
        "【人物认知策略】"
        f"知情边界：{base['knowledge_scope']}"
        f"查档倾向：{mode}"
        f"判断风格：{bias}"
        f"证据铁律：{base['evidence_rule']}"
        "回答建议按「查得事实 → 本人判断 → 可行行动」组织。"
    )
...
def create_minister_agent(
    ...
    if is_consort:
        ...
        # 月度动态上下文只保留轻索引、人物认知策略和必要旧事；具体盘面事实由只读工具按需查询。
        court_brief = build_court_brief(context)
        use_roster_tool = True
        use_army_tool = True
        ...
        intel_tool_index = build_intel_tool_index()
        cognitive_strategy = build_character_cognitive_strategy(character)
        monthly_block_parts = [
