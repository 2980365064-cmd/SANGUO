#!/usr/bin/env python3
"""FastAPI web entry for the 208 Liu Bei campaign.
...
from ming_sim.government import appoint_office, office_effect
...
from ming_sim.intelligence import get_character_intel_level, visible_character_profile
from ming_sim.historical_events import historical_timeline_preview
from ming_sim.battle import preview_battle as build_battle_preview
from ming_sim.monthly_report import build_monthly_report
from ming_sim.national_focus import start_focus, start_region_investment, focus_points
from ming_sim.registry import _BASE_SKILLS
from ming_sim.tools import build_evidence_gate_context, build_minister_tools
...
_AGNO_SKILL_DESCRIPTIONS = {
    ...
    "decree-drafting": "把已定处置整理成军府方略草案并入档。",
...
_TOOL_LABELS = {
    "query_world_index": "查案卷目录",
    "query_military_intel": "查军事案卷",
    "query_internal_intel": "查内政案卷",
    "query_diplomacy_intel": "查外交案卷",
    "query_personnel_intel": "查人事案卷",
    "query_secret_intel": "查密令暗流",
    "query_monthly_intel": "查月报章节",
...
_TOOL_DESCRIPTIONS = {
    "query_world_index": "查看军事、内政、外交、人事、密令、月报等可查案卷目录。",
    "query_military_intel": "查询军队编制、兵力、补给、疲劳、军械、战役和战俘。",
    "query_internal_intel": "查询郡县粮仓、城防、民心、动乱、道路和投资。",
    "query_diplomacy_intel": "查询势力关系、条约、使臣进展和违约风险。",
    "query_personnel_intel": "查询人物状态、官职、任事效果和职位空缺。",
    "query_secret_intel": "查询自己承办密令详情或公开暗流摘要。",
    "query_monthly_intel": "按章节查询每月军政总计。",
    ...
    "propose_directive": "把已定处置方案拟成军府方略草稿呈阅。",
...
class RegionInvestmentRequest(BaseModel):
    category: str


class OfficeAppointmentRequest(BaseModel):
    character_name: str
    target_id: str = ""
...
class ScenarioCreateRequest(BaseModel):
    name: str
    description: str = ""
    # 来源：""/"blank"=空白；"__default__"=复制默认建安十三年；其余=复制该剧本 id 的三件套。
    copy_from: str = ""
...
_ABILITY_LABELS = {
    "martial": "武力", "leadership": "统率", "intelligence": "智略",
    "politics": "政治", "diplomacy": "外交", "charisma": "魅力",
}
_PERSONALITY_LABELS = {
    "loyalty": "忠诚", "integrity": "节义", "ambition": "野心",
    "courage": "胆略", "closeness_to_liu_bei": "对刘备亲密度",
}
...
class WebGame:
    ...
    def __init__(self, fresh: bool = False) -> None:
        ...
        _DEFAULT_FAVORITES = {"关羽", "张飞", "赵云", "诸葛亮", "糜竺"}
    ...
    def apply_llm_config(
        ...
        # 只重建 Agent，不重跑人物登场/死亡、阶段切换或自动存档。
    ...
    def _runtime_query_flags(self) -> tuple[bool, bool]:
        active_char_count = sum(
            ...
            and getattr(ch, "power_id", "liu_bei") == "liu_bei"
    ...
    def public_character(self, character: Character) -> Dict[str, Any]:
        ...
        return {
            ...
            "abilities": visible_profile["abilities"],
            "personality": visible_profile["personality"],
            "intel_level": visible_profile["intel_level"],
            "core_tier": character.core_tier,
    ...
    def _decode_json(raw: object, fallback: object) -> object:
        try:
            return json.loads(str(raw or json.dumps(fallback, ensure_ascii=False)))
        except (TypeError, ValueError, json.JSONDecodeError):
            return fallback

    @staticmethod
    def _translated_visible_group(group: Dict[str, Any], labels: Dict[str, str]) -> Dict[str, Any]:
        return {
            "visibility": str(group.get("visibility") or "assessment"),
            "values": {
                labels[key]: value
                for key, value in dict(group.get("values") or {}).items()
                if key in labels
            },
        }

    def sanguo_character_payload(self, character: Character) -> Dict[str, Any]:
        """三国人物公开档案；动态人格必须先经过情报裁剪。"""
        status, status_reason = self.db.get_character_status(character.name)
        row = self.db.conn.execute(
            "SELECT power_id, origin, archived, location, portrait_id FROM characters WHERE name=?",
            (character.name,),
        ).fetchone()
        power_id = str((row["power_id"] if row else "") or character.power_id or "")
        visible = visible_character_profile(
            replace(character, power_id=power_id),
            get_character_intel_level(self.db, character.name),
        )
        ...
        return {
            ...
            "office_type": character.office_type,
            "political_group": character.faction,
            "power_id": power_id,
            "location": str((row["location"] if row else "") or character.location),
            "status": status,
            "status_reason": status_reason,
            "core_tier": character.core_tier,
            "traits": list(character.personal_skills),
            "style": character.style,
            "summary": character.summary,
            "portrait_id": str((row["portrait_id"] if row else "") or character.portrait_id),
            "abilities": self._translated_visible_group(visible["abilities"], _ABILITY_LABELS),
    ...
    def sanguo_map_payload(self) -> Dict[str, Any]:
        ...
        for army in self.db.conn.execute(
            "SELECT id, station_node FROM armies WHERE active=1 ORDER BY id"
        ).fetchall():
        ...
        for row in rows:
            ...
            nodes.append({
                ...
                "x": x,
                "y": y,
                "controller": str(row["controlled_by"] or ""),
    ...
    def sanguo_army_payloads(self) -> List[Dict[str, Any]]:
        current_orders = {
            str(item["army_id"]): item for item in self.db.list_army_orders(int(self.state.turn))
        }
        payloads: List[Dict[str, Any]] = []
        for row in self.db.conn.execute("SELECT * FROM armies WHERE active=1 ORDER BY id").fetchall():
            army_id = str(row["id"])
            payloads.append({
                "id": army_id,
                "name": str(row["name"]),
                ...
                "troop_composition": self._decode_json(row["troop_composition"], {}),
                "manpower": int(row["manpower"] or 0),
                "supply": int(row["supply"] or 0),
                "supply_turns": int(row["supply_turns"] or 0),
                "morale": int(row["morale"] or 0),
                "training": int(row["training"] or 0),
                "equipment": int(row["equipment"] or 0),
                "mobility": int(row["mobility"] or 0),
                "loyalty": int(row["loyalty"] or 0),
                "fatigue": int(row["fatigue"] or 0),
                "experience": int(row["experience"] or 0),
                "discipline": int(row["discipline"] or 0),
                "hazard_turns": int(row["hazard_turns"] or 0),
                "hazard_combat_multiplier": float(row["hazard_combat_multiplier"] or 1.0),
                "hazard_mobility_multiplier": float(row["hazard_mobility_multiplier"] or 1.0),
                "starvation_turns": int(row["starvation_turns"] or 0),
    ...
    def sanguo_diplomacy_payload(self) -> Dict[str, Any]:
        relations = []
        for row in self.db.conn.execute(
            "SELECT * FROM diplomatic_relations ORDER BY power_a, power_b"
        ).fetchall():
            item = dict(row)
            for field, fallback in (
                ("obligations", []), ("territorial_claims", {}), ("marriage_hostages", {})
            ):
                item[field] = self._decode_json(item.get(field), fallback)
            relations.append(item)
        return {"relations": relations, "treaties": self.db.list_diplomacy_treaties()}

    def sanguo_focus_payload(self) -> Dict[str, Any]:
        progress = [
            dict(row) for row in self.db.conn.execute(
                "SELECT * FROM national_focus_progress ORDER BY category, focus_id"
            ).fetchall()
        ]
        effects = [
            dict(row) for row in self.db.conn.execute(
                "SELECT * FROM national_focus_effects ORDER BY focus_id, effect_key"
            ).fetchall()
        ]
        return {
            "definitions": self.content.national_focuses,
            "progress": progress,
            "effects": effects,
            "points_per_turn": {
                category: focus_points(self.db, self.state, category)
    ...
    def preview_battle(self, plan: Dict[str, Any]) -> Dict[str, Any]:
        if not isinstance(plan, dict):
            raise ValueError("战役预览参数必须为对象。")
        attackers = [str(item) for item in (plan.get("attacker_ids") or [])]
        defenders = [str(item) for item in (plan.get("defender_ids") or [])]
        node_id = str(plan.get("node_id") or "").strip()
        if not attackers or not defenders or not node_id:
            raise ValueError("战役预览必须指定攻方、守方和目标节点。")
        return build_battle_preview(self.db, attackers, defenders, node_id)

    def region_detail_payload(self, region_id: str) -> Dict[str, Any]:
        row = self.db.conn.execute("SELECT * FROM regions WHERE id=?", (str(region_id),)).fetchone()
        if row is None:
            raise ValueError(f"地区不存在：{region_id}")
        fiscal = self._decode_json(row["fiscal"], {})
        investment = self.db.conn.execute(
            "SELECT * FROM region_investments WHERE region_id=?", (str(region_id),)
        ).fetchone()
        investment_logs = [
            dict(item) for item in self.db.conn.execute(
                """
                SELECT * FROM region_investment_logs
                WHERE region_id=?
                ORDER BY id DESC
                LIMIT 6
                """,
        ...
        stationed = [
            army for army in self.sanguo_army_payloads()
            if str(army["station_node"]) == str(region_id)
        ]
        return {
            "id": str(row["id"]),
            "name": str(row["name"]),
            ...
            "can_invest": str(row["controlled_by"]) == "liu_bei",
            "population": int(row["population"] or 0),
            "public_support": int(row["public_support"] or 0),
            "unrest": int(row["unrest"] or 0),
            "military_pressure": int(row["military_pressure"] or 0),
            ...
            "status": str(row["status"] or ""),
            "natural_disaster": str(row["natural_disaster"] or ""),
            "human_disaster": str(row["human_disaster"] or ""),
            "fiscal": fiscal,
            "investment": dict(investment) if investment is not None else None,
    ...
    def government_office_payload(self) -> List[Dict[str, Any]]:
        return [
            ...
            for office_key in self.content.sanguo_offices
        ]

    def appoint_government_office(
        self,
        office_key: str,
        character_name: str,
        target_id: str = "",
    ...
    def state_payload(self) -> Dict[str, Any]:
        ...
        title, scene = _GOVERNMENT_SCENES.get(stage, ("左将军", "军府"))
        metric_names = ("军资", "粮秣", "民望", "名分", "军心", "士族支持")
        battles = []
        for row in self.db.conn.execute("SELECT * FROM battles ORDER BY id DESC").fetchall():
            item = dict(row)
            for field, fallback in (
                ("attacker_ids", []), ("defender_ids", []), ("preview", {}),
                ("ai_choice", {}), ("result", {}),
            ):
                item[field] = self._decode_json(item.get(field), fallback)
            battles.append(item)
        families = [
            dict(row) for row in self.db.conn.execute(
                "SELECT * FROM family_relations ORDER BY id"
            ).fetchall()
        ]
        offices = [
            dict(row) for row in self.db.conn.execute(
                "SELECT * FROM government_offices ORDER BY office_key"
            ).fetchall()
        ]
        investments = [
            dict(row) for row in self.db.conn.execute(
        ...
        return {
            ...
            "armies": self.sanguo_army_payloads(),
            "army_orders": self.db.list_army_orders(int(self.state.turn)),
            "sieges": self.db.list_sieges(),
            "battles": battles,
            "diplomacy": self.sanguo_diplomacy_payload(),
            "national_focus": self.sanguo_focus_payload(),
            "region_investments": investments,
            "timeline": historical_timeline_preview(self.db, self.state, months=12),
            "characters": [
                self.sanguo_character_payload(character)
    ...
    def chat_stream(self, minister_name: str, message: str) -> Iterator[Dict[str, Any]]:
        ...
        try:
            ...
            run_input: Any = text
            evidence_context = build_evidence_gate_context(character, self.session.registry.context, text)
            if evidence_context:
                run_input = (
                    f"{evidence_context}\n\n"
                    "【作答约束】你可以继续调用工具补查；若不补查，具体事实只能引用上方证据门内容。\n\n"
                    f"【主公问话】{run_input}"
                )
    ...
    def _court_conclusion_prompt(
        ...
        *,
        ...
        body_fields = (
            "1 已出现的主张；2 已出现的分歧；3 已出现的可行章程；4 仍需主公裁断之处。"
            "若材料不足，就明确说材料尚不足，只归纳已见内容。"
            if visible_only else
            "1 今日主张；2 责任归属；3 阻力/风险；4 下一步可定为军府方略的章程，"
            "语气像军府议事后向主公回奏；正文不要分隔符、不要 JSON、不要旁白。"
        )
        option_rule = (
        ...
        return (
            f"【军议结论】现在必须根据{source_label}给主公一个明确结论。"
            f"{guard}"
            f"先输出 120-220 字结论正文：须含 {body_fields}\n"
            "正文之后，另起一行输出一个 <<DECISION>>{...}<<END>> JSON 块，"
            "给主公 2-3 个可直接拍板的完整军府方略（不是结论摘要，也不是只言片语）：\n"
            '{"title":"≤12字本场抉择名","context":"为何此刻必须主公亲断（30-60字）",'
            '"options":[{"label":"完整军府方略，须含谁来办、办什么、用什么资源/章程，可直接入本月方略","hint":"此案倾向性后果，不写具体数值"}]}\n'
            f"{option_rule}"
            "label 必须是一句完整可执行的军府方略（如「令诸葛亮联络孙权，使关羽整军守夏口，粮秣以本月军府余粮支给」），"
    ...
    def _run_court_conclusion(self, agent: Agent, prompt: str, runner: Any) -> Dict[str, Any]:
        fallback = (
            "本轮军议虽未尽合，然争点已明：请主公择一条主线定为军府方略，并限期诸将回奏。"
            '\n<<DECISION>>{"title":"军议未决","context":"群臣争执未休，需主公亲断方向。",'
            '"options":[{"label":"令诸葛亮会同诸将三日内拟出统一章程回奏，逾期另议经办","hint":"稳妥但费时，恐误事机"},'
            '{"label":"主公当场点将拍板，责成一人专办，馀者协办","hint":"决断果敢，然恐压服不服，留下后患"}]}<<END>>'
        )
    ...
    def _active_court_ministers(self, requested: List[str]) -> List[Character]:
        ...
        source_names = requested or [
            ...
            if c.office_type != "后宫" and self.character_power_id(c) == "liu_bei"
        ]
        for name in source_names:
            c = self.content.characters.get(name)
            if c is None or c.name in seen:
                continue
            if self.character_power_id(c) != "liu_bei":
                continue
    ...
    def court_chat_stream(self, message: str, ministers: List[str]) -> Iterator[Dict[str, Any]]:
        ...
        history = self.db.load_court_chat_history(self.state.turn)
        self.db.append_court_chat_message(self.state.turn, "emperor", "刘备", text)
        try:
            ...
            def emit_pending(force: bool = False) -> Iterator[Dict[str, Any]]:
                ...
                while pending_text:
                    ...
                    if match:
                        ...
                        current_speaker = "刘备" if current_role == "emperor" else (speaker if speaker in allowed else fallback_speaker)
                        current_content = []
                        yield {"type": "speaker", "role": current_role, "speaker": current_speaker}
            ...
            drama_beats = [
                ...
                {
                    "name": "最后冲突",
                    "goal": "若仍未一致，基于本人物立场把冲突整理成主公可拍板的军府方略；若已有一致，就顺势收束但保留本派条件。",
            ...
            script_prompt = (
                ...
                "请靠提示词范例里的默会知识排一整场廷议：谁发难、谁打断、谁回嘴、谁暗伤、谁补台、谁逼主公裁断，由你按人物画像和盘面决定。\n"
                "所有被召入臣将都应参与；至少两人要二次发言形成回嘴。若主公发言含【主公插话，打断当前廷议并扭转话题】，立刻围绕新问转向。\n"
                "不要替主公写台词；不要输出分析或旁白；不要输出朝议结论。只按 <<<臣:姓名>>> 分隔符输出群臣台词。"
            )
    ...
    def court_chat_summary(self, messages: List[Dict[str, str]]) -> Dict[str, Any]:
        ...
        for message in messages[-40:]:
            ...
            speaker = str(message.get("speaker") or "").strip() or ("刘备" if role == "emperor" else "未知")
    ...
    def suggestions_for(self, character: Character) -> List[Dict[str, str]]:
        ...
        if "query_military_intel" in runtime_ids or "query_army_roster" in runtime_ids:
            extra_suggestions.append({"label": "查军情", "text": "先查军事案卷，看看我军编制、补给、疲劳与敌情，再议打不打。"})
        if "query_internal_intel" in runtime_ids:
            extra_suggestions.append({"label": "查郡县", "text": "查一下江夏、江陵等郡县的粮仓、民心、道路和投资进度。"})
...
class OngoingPlanPatchRequest(BaseModel):
    status: str = ""
    assignee: str = ""
    last_result: str = ""
    next_check_turn: int = 0
    progress: int = -1


class EnvoyMissionRequest(BaseModel):
    target_power: str
    envoy: str
    goal: str
    boundaries: str = ""
...
async def api_preview_battle(request: BattlePreviewRequest) -> Dict[str, Any]:
    try:
        return get_game().preview_battle(request.dict())
...
async def api_region_detail(region_id: str) -> Dict[str, Any]:
    try:
        return get_game().region_detail_payload(region_id)
    except ValueError as error:
...
@app.get("/api/action_intents")
async def api_action_intents(status: str = "") -> Dict[str, Any]:
    return {"intents": get_game().db.list_action_intents(status=status)}
...
async def api_ongoing_plans(status: str = "") -> Dict[str, Any]:
    return {"plans": get_game().db.list_ongoing_plans(status=status)}


@app.patch("/api/ongoing_plans/{plan_id}")
async def api_update_ongoing_plan(plan_id: int, request: OngoingPlanPatchRequest) -> Dict[str, Any]:
    changes: Dict[str, Any] = {}
    if request.status:
        changes["status"] = request.status
    if request.assignee:
        changes["assignee"] = request.assignee
    if request.last_result:
        changes["last_result"] = request.last_result
    if request.next_check_turn > 0:
        changes["next_check_turn"] = request.next_check_turn
    if request.progress >= 0:
        changes["progress"] = request.progress
    try:
        return {"plan": get_game().db.update_ongoing_plan(plan_id, **changes)}
    except ValueError as error:
        raise HTTPException(status_code=404, detail=str(error)) from None


@app.get("/api/month_agenda")
async def api_month_agenda() -> Dict[str, Any]:
    game = get_game()
    return {"items": game.db.month_agenda(game.state)}


@app.get("/api/monthly-report")
async def api_monthly_report() -> Dict[str, Any]:
    game = get_game()
...
@app.get("/api/envoys")
async def api_envoys(status: str = "") -> Dict[str, Any]:
    return {"missions": get_game().db.list_envoy_missions(status=status)}
...
async def api_create_manual_issue(request: ManualIssueCreateRequest) -> Dict[str, Any]:
    ...
    # 建筑预埋校验地区：玩家选的 region_id 必须是刘备控制或拥有驻军权的地区。
    ...
    if bld is not None:
        ...
        if str(row["controlled_by"]) != "liu_bei":
            raise HTTPException(status_code=400, detail="只能在刘备军府可控地区营建，不能建到外部势力地盘。")
...
async def api_set_issue_authorization(issue_id: int, request: IssueAuthorizationRequest) -> Dict[str, Any]:
    """给 active 局势的承办人设授权：追加专款（指定出库）、开/关非常处置权。
    承办人此后每月自主从专款推进，不必主公反复下达方略。"""
...
def _require_chat_capable_character(character_name: str) -> None:
    if character_name in get_game().session.temporary_characters:
        return
    if character_name not in get_game().content.characters:
        raise HTTPException(status_code=404, detail=f"未找到人物：{character_name}")
    archived_row = get_game().db.conn.execute("SELECT archived FROM characters WHERE name=?", (character_name,)).fetchone()
    if archived_row is not None and int(archived_row["archived"] or 0):
        raise HTTPException(status_code=409, detail=f"{character_name}已归档，请先恢复。")
    if get_game().character_power_id(get_game().content.characters[character_name]) != "liu_bei":
        raise HTTPException(status_code=409, detail=f"{character_name}不在刘备军中，当前无法直接召见。")
    status, reason = get_game().db.get_character_status(character_name)
    if status in {"dead", "offstage"}:
        label = _STATUS_LABEL_WEB.get(status, status)
        detail = f"{character_name}已{label}，无法召见。" + (reason or "")
        raise HTTPException(status_code=409, detail=detail.strip())


@app.get("/api/characters/{character_name}/dialogue")
async def api_chat_history(character_name: str) -> Dict[str, Any]:
    if character_name not in get_game().content.characters and character_name not in get_game().session.temporary_characters:
        raise HTTPException(status_code=404, detail=f"未找到人物：{character_name}")
    archived_row = get_game().db.conn.execute("SELECT archived FROM characters WHERE name=?", (character_name,)).fetchone()
    if archived_row is not None and int(archived_row["archived"] or 0):
        raise HTTPException(status_code=409, detail=f"{character_name}已归档，请先恢复。")
    character = get_game().session._character(character_name)
    return {
        "character": get_game().sanguo_character_payload(character),
        "history": get_game().chat_history.get(character_name, []),
        "suggestions": get_game().suggestions_for(character) if character.status == "active" else [],
    }


@app.post("/api/characters/{character_name}/secret-order")
async def api_create_secret_order(character_name: str, request: SecretOrderRequest) -> Dict[str, Any]:
    """刘备直接下达密令，不经 LLM，直接落库。"""
    game = get_game()
    character = game.session.content.characters.get(character_name)
    if not character:
        raise HTTPException(status_code=404, detail=f"未找到人物：{character_name}")
    if game.character_power_id(character) != "liu_bei":
        raise HTTPException(status_code=409, detail=f"{character_name}不在刘备军中，无法下达密令。")
    ...
    try:
        order_id = game.db.create_secret_order(
            game.session.state, character_name, title, content, request.tags, deadline_months=request.deadline_months
        )
    except ValueError as exc:
        from fastapi import HTTPException
        raise HTTPException(status_code=409, detail=str(exc)) from None
    return {"order_id": order_id, "character_name": character_name, "title": title, "status": "active"}
